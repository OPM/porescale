...
! comments - sides
      ...
      read(1,*)Em1 ! Young modulus of first solid component
      read(1,*)nu1 ! Poisson's ratio of first 
      read(1,*)Em2 ! Young modulus of second component
      read(1,*)nu2 !Poisson's ratio of second
...
c      ncx=ncx+2
      ...
      rsi=1.d0/rs
c      write(*,*) 'alf1=',alf1,'alf2=',alf2
c      write(*,*) 'bet1=', bet1,'bet2=',bet2
c      pause
...
119   format(19i3)
      ...
      allocate(ind(ncx+1,ncy,ncz),stat=checkstat) ! create medium structure array
      if(checkstat>0) then
       write(*,*)'ERROR! impossible to allocate memory for ind'
       stop
      endif
...
!      in the sample with three components, ind may be > 1
c      do k=1,ncz
c       do j=1,ncy
c        do i=1,ncx+1
...
c      enddo
      ...
      allocate(inda(ncx+2,ncy+1,ncz+1),stat=checkstat) ! create solid point indicator matrix
      if(checkstat>0) then
      ...
      allocate(inds(ncx+2,ncy,ncz),stat=checkstat) ! create solid point indicator matrix
      if(checkstat>0) then
      ...
      inds(1:ncx+2,1:ncy,1:ncz)=inda(1:ncx+2,1:ncy,1:ncz) ! initialize from inda

      deallocate(inda) ! dont need it
      ncx=ncx+2 ! new bounds of the domain
      ncy=ncy
      ncz=ncz
...
! to take into account dl icors should be changed
      ...
      allocate(iicx(ncx+1),stat=checkstat) ! create x-axis periodic boundary conditions
      if(checkstat>0) then
      ...
      iicx=(/ncx-1,(i,i=1,ncx-1),1/) ! s.p. bound.cond. ncx+1 - max, ncx+2 - imposible !
...
! Save program parameters in lsm_settings
      ...
      write(19,*)'Young modulus Em1 of component solid 1 = ',Em1
      write(19,*)'Poissons ratio nu1 = ',nu1
      write(19,*)'linear spring parameter alf = ',alf1
      write(19,*)'angular spring parameter bet = ',bet1
      write(19,*)'Young modulus Em1 of component solid 2 = ',Em2
      write(19,*)'Poissons ratio nu2 = ',nu2
      write(19,*)'linear spring parameter alf = ',alf2
      write(19,*)'angular spring parameter bet = ',bet2
      ...
      do j=1,ncy
       do k=1,ncz
        ...
        if(l1.gt.0.and.l2.gt.0)then
         rdat(1:3,l1)=(/-gstrain,0.D0,0.D0/)
         rdat(1:3,l2)=(/gstrain,0.D0,0.D0/)
        endif
       enddo
      enddo
...
c----------------------------------------------------------
     ...
     &                    ncx,ncy,ncz,mijks,ipcx,ipcy,ipcz,
     &                    ind,inds,alf1,bet1,alf2,bet2,Em,nu,exx0)
...
c      real*8    rvirt(3,ns) ! virtual displacement
      ...
      integer ind(ncx-1,ncy,ncz) ! index file
...
c          write(*,*)'flin=',flin,'l=',l,'sbond=',sbond(l),
c&                   'xn1=',xn(1),'xn2=',xn(2),'xn3=',xn(3),
c&                    'ncub=',ncub,'sigmap1=',sigmap(1:3,1,ncub),
c&          'sigmam1=',sigmam(1:3,1,ncub),'sigmap2=',sigmap(1:3,2,ncub),
c&          'sigmam2=',sigmam(1:3,2,ncub),'sigmap3=',sigmap(1:3,3,ncub),
c&          'sigmam3=',sigmam(1:3,3,ncub)
c          pause
          enddo
...
c            write(*,*)'dfi=',dfi,'anf=',anf(7:9,ang(6,l)),'l=',l,
c&                    'abond=',abond(l),
c&                   'xnj1=',xnj(1),'xnj2=',xnj(2),'xnj3=',xnj(3),
c&                    'xnk1=',xnk(1),'xnk2=',xnk(2),'xnk3=',xnk(3),
c&                    'ncub=',ncub,'sigmap1=',sigmap(1:3,1,ncub),
c&          'sigmam1=',sigmam(1:3,1,ncub),'sigmap2=',sigmap(1:3,2,ncub),
c&          'sigmam2=',sigmam(1:3,2,ncub),'sigmap3=',sigmap(1:3,3,ncub),
c&          'sigmam3=',sigmam(1:3,3,ncub)
...
1984  format(9(1X,E15.8))
      ...
      rncx=ncx-2
      Exxxx=asigmap(1,1)/(vcell*exx0/rncx)
      Eyyxx=asigmap(2,2)/(vcell*exx0/rncx)
      Eyyzz=asigmap(3,3)/(vcell*exx0/rncx)
      ...
      write(113,*)'Global strain exx0 = ',exx0
      ...
      write(113,*)'Exxxx = ',Exxxx
      write(113,*)'Eyyxx = ',Eyyxx
      write(113,*)'Eyyzz = ',Eyyzz
      close(113)
         ...
         if(isc.eq.0)then ! there is a cube
          do l=1,6  !(6 possible sides) check if cube side is a surfase
           ...
           do m=1,4 !(4 points to check)
            ...
            isg=isg+ind(nspc(1,m),nspc(2,m),nspc(3,m))
           enddo
