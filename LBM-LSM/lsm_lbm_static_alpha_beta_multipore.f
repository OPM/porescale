...
!	integer   nthread
	...
	integer   iv(3,19) ! discrete velocities (integer numbers)
      ...
      real*8 alf ! alpha linear spring constant LSM
      real*8 bet ! beta angular spring constant LSM
...
! LSM_START
	...
	integer, dimension(:), allocatable :: layers
      integer*4, dimension(:), allocatable :: layb
      integer*4, dimension(:), allocatable :: lib
      integer*4, dimension(:), allocatable :: lia
...
! LSM code start
      ...
      read(1,*)Em ! Young modulus
      read(1,*)nu ! Poisson's ratio
...
! LSM code start
      ...
      alf=dl*Em/(5.d0-10.d0*nu) ! linear spring constant
      bet=dl*dl*dl*Em*(4*nu-1)/(20.d0*(2*nu*nu+nu-1)) ! angular spring constant
      rsi=1.d0/rs
...
119   format(19i3)
      ...
      do k=1,ncz
       do j=1,ncy
        do i=1,ncx
         if(ind(i,j,k).eq.0)then ! solid element is detected
          inda(i,j,k)=0
          inda(i+1,j,k)=0
          inda(i,j+1,k)=0
          inda(i,j,k+1)=0
...
! to take into account dl icors should be changed
        ...
        if(inds(nx,ny,nz).eq.0) then ! the next point is solid (may be there is a bond)
         ...
         if(j.le.3)then ! first three cases
          if(j.eq.1)then ! 1 0 0 bond
           ncube=ind(nx1,ny1,nz1)+ind(nx1,ny1,iicz(nz1))
     &     +ind(nx1,iicy(ny1),nz1)+ind(nx1,iicy(ny1),iicz(nz1))
          elseif(j.eq.2)then ! 0 1 0 bond
           ncube=ind(nx1,ny1,nz1)+ind(iicx(nx1),ny1,nz1)
     &     +ind(nx1,ny1,iicz(nz1))+ind(iicx(nx1),ny1,iicz(nz1))
          elseif(j.eq.3)then ! 0 0 1 bond
           ncube=ind(nx1,ny1,nz1)+ind(iicx(nx1),ny1,nz1)
     &     +ind(nx1,iicy(ny1),nz1)+ind(iicx(nx1),iicy(ny1),nz1)
          endif
          if(ncube.lt.4)then
          ...
          elseif(j.eq.5)then ! 1 -1 0
           ncube=ind(nx1,iicy(ny1),nz1)+ind(nx1,iicy(ny1),iicz(nz1))
          elseif(j.eq.6)then ! 1 0 1
           ncube=ind(nx1,ny1,nz1)+ind(nx1,iicy(ny1),nz1)
          elseif(j.eq.7)then ! 1 0 -1
           ncube=ind(nx1,ny1,iicz(nz1))+ind(nx1,iicy(ny1),iicz(nz1))
          elseif(j.eq.8)then ! 0 1 1
           ncube=ind(nx1,ny1,nz1)+ind(iicx(nx1),ny1,nz1)
          elseif(j.eq.9)then ! 0 1 -1
           ncube=ind(nx1,ny1,iicz(nz1))+ind(iicx(nx1),ny1,iicz(nz1))
          endif
          if(ncube.lt.2)then
           nbo=nbo+1 ! there is a bond
          endif
     ...
     &     +ind(nx1,iicy(ny1),nz1)+ind(iicx(nx1),iicy(ny1),nz1)
          ...
          if(ncube.lt.2)then
           bconst = (2.D0-ncube)*0.5D0 ! bond constant
           nbo=nbo+1 ! there is a bond
           bon(1,nbo)=i ! point 'from'
           bon(2,nbo)=mijks(nx,ny,nz) ! point 'to'
           bon(3,nbo)=j ! bond vector index 'from'->'to'
        ...
        if(inds(nxj,nyj,nzj).eq.0.and.inds(nxk,nyk,nzk).eq.0) then ! may be there is an angle
         if(nta(1,j).gt.7.and.nta(2,j).gt.7)then ! only one cube possible
          ...
          if(jv(1).eq.0.and.kv(1).eq.0)then ! yz plane
           ...
           iny1=iicy(min(ny1,npj(2),npk(2))+1)
           inz1=iicz(min(nz1,npj(3),npk(3))+1)
           iscube = ind(inx1,iny1,inz1)+ind(iicx(inx1),iny1,inz1)
          elseif(jv(2).eq.0.and.kv(2).eq.0)then ! xz plane
           inx1=iicx(min(nx1,npj(1),npk(1))+1)
           iny1=iicy(min(ny1,npj(2),npk(2))+1)
           inz1=iicz(min(nz1,npj(3),npk(3))+1)
           iscube = ind(inx1,iny1,inz1)+ind(inx1,iicy(iny1),inz1)
          elseif(jv(3).eq.0.and.kv(3).eq.0)then ! xy plane
           inx1=iicx(min(nx1,npj(1),npk(1))+1)
           iny1=iicy(min(ny1,npj(2),npk(2))+1)
           inz1=iicz(min(nz1,npj(3),npk(3))+1)
           iscube = ind(inx1,iny1,inz1)+ind(inx1,iny1,iicz(inz1))
          endif
          if(iscube.lt.2)then ! there is an angle
           nna=nna+1 ! there is an angle i-j-k
          endif
         endif
        endif
        ...
        if(inds(nxj,nyj,nzj).eq.0.and.inds(nxk,nyk,nzk).eq.0) then ! may be there is an angle
         if(nta(1,j).gt.7.and.nta(2,j).gt.7)then ! only one cube possible
          ...
          if(iscube.eq.0)then ! there is a cube and so there is an angle
           ...
           abond(nna)=1.D0 ! elastic constant
          endif
         else ! two cubes possible
          jv=iv(1:3,nta(1,j))
          kv=iv(1:3,nta(2,j))
          ...
          if(iscube.lt.2)then ! there is an angle
           ...
           ang(5,nna)=nta(2,j) ! i-k vector index
           ang(6,nna)=j ! angle indentificator
           abond(nna)=(2.D0-iscube)*0.5D0 ! elastic constant
          endif
         endif
        endif
       enddo
...
c      write(1313,*)'initialization end = ',time
     ...
     &  nbo,bn,bon,bx,vb,alf,bet,nna,vn,ang,vx,vv,anf,rsi,mijks,
...
c___________________________________________________
     ...
     &                    lib,lia)
      ...
      integer*4 lib(nthread2+1),lia(nthread2+1)
...
c     real*8  iv2(3,18),cn(18)
      ...
      real*8    bx(3,9)     ! xn/|rn|
      real*8    vb(3,9)     ! bond vectors (real numbers)
      real*8    alf ! alpha linear spring constant
      real*8    bet ! beta angular spring constant
...
c      real*8    rvirt(3,ns) ! virtual displacement
      ...
      real*8    alf,bet              ! elastic constants
      ...
      do i=1,ncx
       do j=1,ncy
        do k=1,ncz
         ...
         if(isc.eq.0)then ! there is a cube, start calculate stress tensor
          ncub=ncub+1
         endif
      ...
      do i=1,ncx
       do j=1,ncy
        do k=1,ncz
         ...
         if(isc.eq.0)then ! there is a cube, start calculate strain and stress tensors
          ncub=ncub+1
